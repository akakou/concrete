...
// Views are here as there is no specification for them, as they are just GlweCiphertext entities
// that do not own their memory

/// A structure representing a GLWE ciphertext view, that does not own its memory, with 32 bits
...
/// A structure representing a GLWE ciphertext view, that does not own its memory, with 64 bits
/// of precision. This view is not Clone as Clone for a slice is not defined. It is not
/// Deserialize either, as Deserialize of a slice is not defined. Immutable variant.
#[cfg_attr(feature = "serde_serialize", derive(Serialize))]
#[derive(Debug, PartialEq)]
